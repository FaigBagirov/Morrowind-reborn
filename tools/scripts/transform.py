...
LUA_STORE = {
    ...
    "MGEF": "magicEffects",
    # `potions` is one of the fifteen sub-packages WO0 enumerated at runtime, so
    # the store exists. Whether a potion's `name` takes a write is unprobed -
    # the same footing MGEF `description` is on. Without this line the fifteen
    # renamed potions would fall through to the plugin half, which is the
    # expensive route and the one that touches saves, for records the load
    # context can almost certainly reach. The write is guarded, and the log
...
def lua_quote(text):
    ...
    Three things disqualify the long bracket, and only the first is obvious:

    * `]]` inside the text closes it early;
    * a text *ending* in `]` closes it early too, because its own bracket plus
      the closer makes `]]]`. The spell id `absorb spell points [ranged]` is the
      case that found this - the file parsed until the first rule set that
      targeted a record with a bracket in its name;
    * a leading newline, which Lua silently swallows inside a long bracket.
    """
...
def emit_lua_rules(rules, targets, table_hash, path, ceiling=None):
    lines = [
    ...
    for r in rules:
        ...
        # The scope has to travel with the rule. Without it the Lua half
        # applies every rule to every target, and the two halves describe
        # different mods: the magicka rules are restricted to four record types
        # so that books and dialogue keep the word, and the first build to carry
        # them renamed it inside books as well. The equivalence test caught it.
        scope = ""
        for name, values in (("types", r.types), ("fields", r.fields)):
            if values != ["*"]:
                items = ", ".join('["%s"] = true' % v for v in sorted(values))
                scope += f" {name} = {{ {items} }},"
        lines.append(
            "        { id = %s, pattern = %s, replacement = %s,"
            " left = %s, right = %s, case = %s,%s%s },"
            % (lua_quote(r.id), lua_quote(r.pattern), lua_quote(r.replacement),
               "true" if r.left else "false", "true" if r.right else "false",
               lua_quote(r.case), scope, excl)
        )
    lines.append("    },")
    ...
    lines.append("    },")
    # The longest string vanilla ships in each field, so the load context can
    # hold a rewrite to the same measure the transform does. Its own guard was
    # "never longer than what it replaces", which would have silently dropped
    # the eighteen strings that legitimately grow.
    lines.append("    ceiling = {")
    for (code, field), limit in sorted((ceiling or {}).items()):
        lines.append("        { %s, %s, %d },"
                     % (lua_quote(code), lua_quote(field), limit))
    lines.append("    },")
    lines.append("}")
...
def main():
    ...
    # The length rule protects UI fields from overflow. "Did it grow" is a
    # proxy for that, and the real question is whether the string still fits -
    # so a string that grew is held against the longest one vanilla already
    # ships in the same record type and field, measured off the masters above.
    # A hand-written book is exempt: book text paginates, so it may run longer
    # on purpose. The exemption is narrow - authored NAME and DESCRIPTION
    # fields still have to fit.
    grew = [r for r in rows
            if r["length_delta"] > 0
            and not (r["rules"] == "HAND-WRITTEN" and r["field"] == "text")]
    over = [r for r in grew
            if r["produced"] > ceiling[(r["record_type"], r["field"])]]
    if grew:
        widest = collections.defaultdict(int)
        for r in grew:
            key = (r["record_type"], r["field"])
            widest[key] = max(widest[key], r["produced"])
        print(f"{len(grew)} fields grew, {len(over)} over their field's "
              f"vanilla ceiling:")
        for key in sorted(widest):
            print(f"    {key[0]} {key[1]}: longest produced {widest[key]}, "
                  f"longest vanilla {ceiling[key]}")
    if over:
        for r in over[:5]:
            print("  REFUSED: over the ceiling:", r["record_type"],
                  r["record_id"], r["field"], r["produced"], ">",
                  ceiling[(r["record_type"], r["field"])])
        raise SystemExit(f"{len(over)} fields exceed what vanilla already "
                         f"ships in the same field - refusing to write")
    ...
    emit_lua_rules(rules, lua_targets, table_hash, rules_lua, ceiling)
