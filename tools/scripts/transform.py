...
# Stamped into every plugin we emit, and how a rebuild recognises its own
# previous output in a load order. See is_ours() below.
MADE_BY = b"Generated by transform.py"
...
def main():
    ...
    if args.profile == "momw":
        ...
        # Ask the file who made it. Two weaker tests were tried and both let
        # our own build back in:
        #
        #   by name - --out-name defaults to "scifi-rewrite" while the
        #     installed artifact is "scifi-rewrite-momw.esp", so a run that
        #     forgot the flag read its own output as source;
        #   by directory - the play profile names the *other* worktree's build
        #     directory, so ours never matched.
        #
        # The plugin stamps "Generated by transform.py" into its TES3 header,
        # which no third-party mod carries and neither a rename nor a move can
        # remove. Both failures were silent: the build exits 0 and writes a
        # valid, tiny plugin. The diff report fell from 427 rows to 212.
        def is_ours(path):
            try:
                with open(path, "rb") as fh:
                    return MADE_BY in fh.read(512)
            except OSError:
                return False

        kept = [(n, p) for n, p in plugins if not is_ours(p)]
