...
Beside the table sits `tools/rules/frozen-records.csv`: whole records the transform never touches, whatever the rules say, each with its reason. The distinction is worth keeping — `exclude_records` excuses one record from one rule; the frozen list excuses a record from **all** of them, because the right text for it is written rather than derived. The first entries are `bk_AedraAndDaedra` and its unique copy, the game's own explanation of what the words mean. Substituting inside it yields *"Zenar" means, roughly, "not our ancestors"*, which is false in our fiction — `Zenar` is what they call themselves. Explaining a word and replacing it everywhere are opposite operations, and the collision landed on precisely the book that has to teach the distinction. Hand-written replacement drafted in `tools/handwritten/`.
